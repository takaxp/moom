...
For macOS users, the default margin is =(23 0 0 0)=, but for other system types, it is not configured when you activate =moom-mode=. So please call =moom-screen-margin= with an appropriate list for your environment.
...
Entering quickly to the frame maximized presentation with [[https://github.com/takaxp/org-tree-slide][org-tree-slide.el]]. No additional settings are required. Just =M-x moom-toggle-frame-maximized=.
