...
Note - This module has basically been tested on [[https://www.apple.com/macos/high-sierra/][macOS High Sierra]], [[https://www.apple.com/jp/macos/catalina/][Catalina]], and [[https://www.apple.com/jp/macos/big-sur/][BigSur]] with a Full HD (1920x1080) single monitor or two Full HD monitors. Additionally, tested in Windows 10 and Linux (Ubuntu 20.04, CentOS 8.2, and openSUSE 15.1) with GTK, but they still have some issues.

Note - This module supports multi monitors for macOS and Windows 10. To enable the feature, please configure =moom-multi-monitors-support= to non-nil and call =moom-identify-current-monitor=. The method will be automatically called in =moom-move-frame-right= and =moom-move-frame-left=. see [[https://github.com/takaxp/moom/issues/12#issuecomment-652237962][issue-12]].
...
For macOS users, the default margin is =(23 0 0 0)=. And for other system types, it is also automatically configured using combination of =frame-monitor-geometry= and =frame-monitor-workarea= when you activate =moom-mode=. But the actual work area is probably depends on user environment.

You can control the actual active region on your screen by setting =moom-user-margin=.
...
Each value means a margin from the ~top~, ~bottom~, ~left~, and ~right~ edge of your screen. If you want to test some combination of the margins, then =moom-check-user-margin= could be helpful.

#+begin_src emacs-lisp
(moom-check-user-margin '(50 50 50 50)) ;; {top, bottom, left, right}
#+end_src

=moom-check-user-margin= will fill your screen with the provided margins. The tested margins will not be immediately applied so please configure the margins in your init.el.
...
| moom-multi-monitors-support          | nil                                | boolean |
...
#+begin_src emacs-lisp
(with-eval-after-load "moom"
  (moom-recommended-keybindings '(move fit expand fill font reset))) ;; 'all
#+end_src
