...
The dispatcher (=moom-transient-dispatch=) keep the menu window after a command is executed as default. If you like to hide the dispatcher each time, then please configure =moom-transient-dispatch-sticky=.
